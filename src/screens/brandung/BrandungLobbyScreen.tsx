...
export default function BrandungLobbyScreen() {
  ...
  async function joinExistingGame(code: string) {
    ...
    if (g.status !== "LOBBY" && g.status !== "WAITING") { setError("Spiel läuft bereits."); return; }
      ...
      {step === "mode" && (
        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          ...
          {brandungSave && (
            <SavedGameRow
              title="Brandung"
              subtitle={brandungSave.displayLabel ?? `Runde ${(() => { try { return (JSON.parse(brandungSave.gameState) as { round?: number }).round ?? 1; } catch { return 1; } })()}`}
              color={TEAL}
              onResume={() => {
                try {
                  const saved = JSON.parse(brandungSave.gameState) as { players?: unknown[] };
                  const savedAiCount = Math.max(1, (saved.players?.length ?? 2) - 1);
                  navigate("/brandung/game", { state: { mode: "ai", aiCount: savedAiCount, difficulty: brandungSave.difficulty, saveId: brandungSave.id } });
                } catch { /* ignore corrupt save */ }
              }}
              onDelete={() => { deleteGameSave("brandung"); setBrandungSave(null); }}
            />
          )}
