        ...
        <div>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            {[
              ...
              { id: "online" as GameMode, emoji: "🌐", label: "Online",      desc: "Gegen echten Spieler (QR-Code)", disabled: true },
            ].map(m => (
              <button
                key={m.id}
                onClick={() => !m.disabled && setGameMode(m.id)}
                disabled={m.disabled}
                style={{
                  padding: "14px 16px", textAlign: "left",
                  background: gameMode === m.id ? ACCENT + "22" : "var(--surface)",
                  border: `1.5px solid ${gameMode === m.id ? ACCENT : "var(--border)"}`,
                  borderRadius: 12, cursor: m.disabled ? "default" : "pointer",
              ...
              >
                ...
                <div style={{ flex: 1 }}>
                  <div style={{ fontSize: 14, fontWeight: 700, color: gameMode === m.id ? ACCENT : "var(--text)" }}>
                    {m.label}{m.disabled && <span style={{ fontSize: 11, marginLeft: 6, color: "var(--text-muted)" }}>demnächst</span>}
                  </div>
                  <div style={{ fontSize: 12, color: "var(--text-muted)", marginTop: 2 }}>{m.desc}</div>
                </div>
                {gameMode === m.id && <span style={{ color: ACCENT, fontSize: 18 }}>✓</span>}
              </button>
            ))}
          </div>
        </div>
