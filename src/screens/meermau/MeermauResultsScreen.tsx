...
export default function MeermauResultsScreen() {
  const navigate = useNavigate();
  const uid = auth.currentUser?.uid ?? "";
  const [results, setResults] = useState<MeermauResult[]>([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    if (!uid) return;
    const q = query(
      collection(db, "meermauResults"),
      where("playerIds", "array-contains", uid),
      orderBy("finishedAt", "desc"),
      limit(30),
    );
    getDocs(q).then(snap => {
      setResults(snap.docs.map(d => ({ resultId: d.id, ...d.data() }) as MeermauResult));
      setLoading(false);
  ...
  return (
    <div className="screen">
      ...
      <div style={{
        ...
        <button onClick={() => navigate("/meermau/lobby", { replace: true })} style={{
      ...
      {total > 0 && (
        <div style={{ padding: "12px 16px", display: "flex", gap: 10 }}>
          ...
          <StatBox label="Gewonnen" value={wins} color={VIOLET} />
          <StatBox label="Winrate" value={`${total > 0 ? Math.round((wins / total) * 100) : 0}%`} color={wins > total / 2 ? VIOLET : undefined} />
        </div>
      )}
            ...
            {results.map(r => {
              ...
              return (
                <div key={r.resultId} className="card" style={{
                  ...
                  border: `1px solid ${isWin ? "rgba(124,58,237,0.3)" : "var(--border)"}`,
                  background: isWin ? "rgba(124,58,237,0.06)" : "var(--surface)",
                }}>
                  <div style={{ fontSize: 28 }}>{isWin ? "🏆" : "🂠"}</div>
                  <div style={{ flex: 1 }}>
                    <div style={{ fontSize: 13, fontWeight: 700, color: isWin ? VIOLET : "var(--text)" }}>
